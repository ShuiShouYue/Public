...
public class ApplyRefuseTest4 {
    ...
    @Test(testName ="登录专家账号，专家审核不通过并上传材料",priority = 6)
    ...
    @Test(testName ="登录省市场监管局账号，综合审核不通过",priority = 7)
    ...
    @Test(testName ="综合审核不通过，登录省级账号，输入姓名，查询用户流转节点，验证流转节点是,已归档（不通过）",priority = 8)
    //查询
